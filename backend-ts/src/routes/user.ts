...
import { ComparisonRequest, BeerListResponse, UserBeerResponse, BeerResponse } from '../models/types';

const router = express.Router();

// Elo rating calculation function
const calculateEloRating = (currentRating: number, opponentRating: number, won: boolean, kFactor: number = 32) => {
...
router.get('/lists', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const db = getDatabase();
    const lists = await db.collection('beer_lists').find({ user_id: new ObjectId(req.user._id) }).toArray();
    
    const listResponses: BeerListResponse[] = lists.map(list => ({
      _id: list._id.toString(),
      user_id: list.user_id.toString(),
      name: list.name,
      beers: list.beers.map((beer: any) => ({
...
router.get('/lists/:list_name', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    for (const beer of list.beers) {
      if (ObjectId.isValid(beer.beer_id.toString())) {
        beerIds.push(new ObjectId(beer.beer_id));
      } else {
    ...
    const beersWithDetails = list.beers
      .filter((userBeer: any) => {
        ...
        return beer_type ? fullBeer?.type === beer_type : true;
      })
      .map((userBeer: any) => {
        const fullBeer = fullBeers.find(b => b._id.toString() === userBeer.beer_id.toString());
        return {
          ...fullBeer,
          _id: fullBeer?._id.toString(),
          elo_score: userBeer.elo_score,
          comparisons: userBeer.comparisons,
          user_created_at: userBeer.created_at
        };
      })
      .sort((a: any, b: any) => b.elo_score - a.elo_score);
...
router.post('/lists/:list_name/beers/:beer_id', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    let beer;
    if (ObjectId.isValid(beer_id)) {
      beer = await db.collection('beers').findOne({ _id: new ObjectId(beer_id) });
    } else {
    ...
    const existingBeer = list.beers.find((b: any) => {
    ...
    if (existingBeer) {
      ...
      await db.collection('beer_lists').updateOne(
        { user_id: new ObjectId(req.user._id), name: list_name },
        { $push: { beers: newUserBeer } as any }
      );
...
router.post('/compare', authenticate, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { beer1_id, beer2_id, winner_id, list_name }: ComparisonRequest = req.body;
    const db = getDatabase();
    ...
    if (!beer1 || !beer2) {
      ...
      if (!beer1) {
        let foundBeer;
        if (ObjectId.isValid(beer1_id)) {
          foundBeer = await db.collection('beers').findOne({ _id: new ObjectId(beer1_id) });
        } else {
          foundBeer = await db.collection('beers').findOne({ _id: beer1_id });
        }
        if (foundBeer) missingBeers.push(foundBeer);
      }
      
      if (!beer2) {
        let foundBeer;
        if (ObjectId.isValid(beer2_id)) {
          foundBeer = await db.collection('beers').findOne({ _id: new ObjectId(beer2_id) });
        } else {
      ...
      // Update references
      const updatedBeer1 = updatedList.beers.find((b: any) => {
        const beerIdStr = b.beer_id.toString();
        return beerIdStr === beer1_id || beerIdStr === beer1_id.toString();
      }) || { elo_score: 1000, comparisons: 0 };
      const updatedBeer2 = updatedList.beers.find((b: any) => {
        const beerIdStr = b.beer_id.toString();
        return beerIdStr === beer2_id || beerIdStr === beer2_id.toString();
      }) || { elo_score: 1000, comparisons: 0 };
